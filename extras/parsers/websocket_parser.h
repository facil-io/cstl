...
static inline __attribute__((unused)) uint64_t websocket_wrapped_len(
    uint64_t len);
 ...
 */
inline static __attribute__((unused)) uint64_t websocket_client_wrap(
    void *target,
    void *msg,
    uint64_t len,
    unsigned char opcode,
 ...
 */
inline static struct websocket_packet_info_s websocket_buffer_peek(
    void *buffer,
    uint64_t len);
 ...
 */
inline static __attribute__((unused)) uint64_t websocket_consume(
    void *buffer,
    uint64_t len,
    void *udata,
...
void websocket_xmask(void *msg, uint64_t len, uint32_t mask) {
  if (len > 7) {
    { /* XOR any unaligned memory (4 byte alignment) */
      ...
      switch (offset) {
      case 3: ((uint8_t *)msg)[2] ^= ((uint8_t *)(&mask))[2];
      /* fallthrough */
      case 2: ((uint8_t *)msg)[1] ^= ((uint8_t *)(&mask))[1];
      /* fallthrough */
      case 1:
  ...
  switch (len) {
  case 7: ((uint8_t *)msg)[6] ^= ((uint8_t *)(&mask))[2];
  /* fallthrough */
  case 6: ((uint8_t *)msg)[5] ^= ((uint8_t *)(&mask))[1];
  /* fallthrough */
  case 5: ((uint8_t *)msg)[4] ^= ((uint8_t *)(&mask))[0];
  /* fallthrough */
  case 4: ((uint8_t *)msg)[3] ^= ((uint8_t *)(&mask))[3];
  /* fallthrough */
  case 3: ((uint8_t *)msg)[2] ^= ((uint8_t *)(&mask))[2];
  /* fallthrough */
  case 2: ((uint8_t *)msg)[1] ^= ((uint8_t *)(&mask))[1];
...
inline static struct websocket_packet_info_s websocket_buffer_peek(
    ...
    uint64_t len) {
  if (len < 2) {
    const struct websocket_packet_info_s info = {0 /* packet */,
                                                 2 /* head */,
                                                 0 /* masked? */};
    return info;
  }
  ...
  case 127:
    if (len < 10)
      return (
          struct websocket_packet_info_s){0, (uint8_t)(10 + mask_l), mask_f};
    {
      uint64_t msg_len = websocket_str2u64(((uint8_t *)buffer + 2));
      if (msg_len >> 62)
        return (struct websocket_packet_info_s){0, 0, 0};
      return (struct websocket_packet_info_s){msg_len,
                                              (uint8_t)(10 + mask_l),
                                              mask_f};
    }
  default:
