...
Writes data to the end of the string in the `fio_str_info_s` struct, returning an updated `fio_str_info_s` struct.
...
Similar to `fio_str_info_write`, only replacing a sub-string or inserting a string in a specific location.
...
A machine native CPU word aligned `memcpy` / `memmove` fallback implemented in a portable way (no SSE or CPU specific instructions).

Requires either a CPU that doesn't care about memory alignment or addresses to be aligned on native memory boundary address for a "word" (`size_t`, usually 8 bytes on 64 bit machines).
