## FIO_IPC - Inter-Process Communication
...
**Note**: Requires a shared secret via the `FIO_SECRET` environment variable. Returns NULL if using a random secret.
...
if (!listener) {
  fprintf(stderr, "Failed to start cluster - check FIO_SECRET env var\n");
}
```
...
```c
SFUNC const char *fio_ipc_url(const char *url);
```
...
- `url` - Ignored (reserved for future use)

**Returns:** The current IPC socket URL string.

```c
const char *url = fio_ipc_url(NULL);
printf("IPC socket: %s\n", url);
...
int main(void) {
  ...
  /* Start cluster listener (requires FIO_SECRET env var) */
  if (!fio_ipc_cluster_listen(9000)) {
    fprintf(stderr, "Cluster disabled - set FIO_SECRET for multi-machine support\n");
  }
