...
static const uint8_t test_minimal_cert[] = {
    /* clang-format off */
    0x30, 0x82, 0x01, 0x2A, /* Certificate SEQUENCE (298 bytes) */
    0x30, 0x81, 0xD5,       /* TBS Certificate SEQUENCE (213 bytes) */
    ...
    0x30, 0x5C,             /* SubjectPublicKeyInfo SEQUENCE (92 bytes) */
    0x30, 0x0D, 0x06, 0x09, 0x2A, 0x86, 0x48, 0x86, 0xF7, 0x0D, 0x01, 0x01,
    0x01, 0x05, 0x00,       /* Algorithm = rsaEncryption */
    0x03, 0x4B, 0x00,       /* BIT STRING (74 bytes, 0 unused) */
    ...
    0x46, 0x3A, 0x87, 0x20, 0x42, 0x36, 0x83,
...
int main(void) {
  FIO_LOG_INFO("Certificate size: %zu bytes", sizeof(test_minimal_cert));
  ...
  FIO_LOG_INFO("Step 1: cert_seq tag=0x%02X len=%zu (is_tag=0x30: %d)", 
               cert_seq.tag, cert_seq.len, fio_asn1_is_tag(&cert_seq, FIO_ASN1_SEQUENCE));
  ...
  FIO_LOG_INFO("Step 2a: TBS tag=0x%02X len=%zu", tbs.tag, tbs.len);
  ...
  FIO_LOG_INFO("Step 2c: sig_alg tag=0x%02X len=%zu", sig_alg.tag, sig_alg.len);
  
  if (fio_asn1_iterator_next(&cert_it, &sig_value) != 0) {
    FIO_LOG_ERROR("Step 2d: Failed to get sig_value");
    return 1;
  }
  FIO_LOG_INFO("Step 2d: sig_value tag=0x%02X len=%zu", sig_value.tag, sig_value.len);
  ...
  FIO_LOG_INFO("Step 3: Signature len=%zu unused=%d", bit_len, unused);
  ...
  FIO_LOG_INFO("Step 4a: First elem tag=0x%02X len=%zu", elem.tag, elem.len);
  
  int version = 0;
  if (fio_asn1_is_context_tag(&elem, 0)) {
    FIO_LOG_INFO("Step 4b: Found version tag [0]");
    ...
    if (fio_asn1_parse(&version_int, elem.data, elem.len)) {
      ...
      if (fio_asn1_parse_integer(&version_int, &ver) == 0) {
        version = (int)ver;
        FIO_LOG_INFO("Step 4b: Version = %d", version);
      }
    }
    /* Get serial */
    if (fio_asn1_iterator_next(&tbs_it, &elem) != 0) {
    ...
    FIO_LOG_INFO("Step 4c: Serial tag=0x%02X len=%zu", elem.tag, elem.len);
  }
  ...
  FIO_LOG_INFO("Step 4d: Serial verified as INTEGER");
  ...
  FIO_LOG_INFO("Step 5a: TBS sig_alg tag=0x%02X len=%zu", elem.tag, elem.len);
  ...
  FIO_LOG_INFO("Step 6a: Issuer tag=0x%02X len=%zu", elem.tag, elem.len);
  ...
  FIO_LOG_INFO("Step 7a: Validity tag=0x%02X len=%zu", elem.tag, elem.len);
  ...
  FIO_LOG_INFO("Step 8a: Subject tag=0x%02X len=%zu", elem.tag, elem.len);
  ...
  FIO_LOG_INFO("Step 9a: SPKI tag=0x%02X len=%zu", elem.tag, elem.len);
  ...
  FIO_LOG_INFO("Step 10: Parsing public key...");
  ...
  FIO_LOG_INFO("Step 10a: alg_id tag=0x%02X len=%zu", alg_id.tag, alg_id.len);
  
  if (fio_asn1_iterator_next(&spki_it, &pubkey_bits) != 0) {
    FIO_LOG_ERROR("Step 10b: Failed to get pubkey bits");
    return 1;
  }
  FIO_LOG_INFO("Step 10b: pubkey_bits tag=0x%02X len=%zu", pubkey_bits.tag, pubkey_bits.len);
  
  if (fio_asn1_parse_bit_string(&pubkey_bits, &bits, &bit_len, &unused) != 0) {
    FIO_LOG_ERROR("Step 10c: Failed to parse pubkey BIT STRING");
    return 1;
  }
  FIO_LOG_INFO("Step 10c: pubkey bit_len=%zu unused=%d", bit_len, unused);
  ...
  FIO_LOG_INFO("Step 10d: RSA seq tag=0x%02X len=%zu", rsa_seq.tag, rsa_seq.len);
  ...
  FIO_LOG_INFO("Step 10f: n tag=0x%02X len=%zu", n_elem.tag, n_elem.len);
  ...
  FIO_LOG_INFO("Step 10h: e tag=0x%02X len=%zu", e_elem.tag, e_elem.len);
  ...
  FIO_LOG_INFO("All manual steps passed! Now testing full parser...");
  ...
  FIO_LOG_INFO("X.509 parse succeeded!");
  return 0;
}
