...
/* I can't seem to remember which ones I actually use... */
#include <ctype.h>
#include <errno.h>
#include <limits.h>
#include <pthread.h>
...
#define TEST_TIME          20 /* test time in seconds 0 == infinity */
...
int main(int argc, char const *argv[]) {
  ...
  case OPENFILE_LIMIT:
    ASSERT_COND(0,
                "FAILED to connect to %s:%s - no open files available?",
                address,
                port);
    break;
  ...
  fprintf(stderr,
          "* Starting %zu attack loops, with %zu bytes per request.\n",
          ATTACKERS,
          MSG_LEN / REQ_PER_MSG);
  size_t thread_count = 0;
  pthread_t *threads = calloc(sizeof(*threads), ATTACKERS + 1);
  ASSERT_COND(threads, "couldn't allocate memory for thread data store");
  ...
  fprintf(stderr,
          ...
          ATTACKERS,
          total_requests,
          (total_requests * (MSG_LEN / REQ_PER_MSG)),
          total_reads,
          total_success,
          total_attempts,
  ...
  if (max_wait > 5 || total_attempts != total_success) {
    ...
    fprintf(stderr,
            "FAILED! the server experienced DoS at least once or "
