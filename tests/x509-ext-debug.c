...
int main(void) {
  ...
  FIO_LOG_INFO("Certificate parsed successfully");
  FIO_LOG_INFO("  Version: %d", cert.version);
  FIO_LOG_INFO("  is_ca: %d", cert.is_ca);
  FIO_LOG_INFO("  has_key_usage: %d", cert.has_key_usage);
  FIO_LOG_INFO("  key_usage: 0x%04X", cert.key_usage);
  
  if (cert.san_dns) {
    FIO_LOG_INFO("  SAN DNS: '%.*s' (len=%zu)", (int)cert.san_dns_len, cert.san_dns, cert.san_dns_len);
  } else {
    FIO_LOG_INFO("  SAN DNS: NULL");
  }
  
  if (cert.subject_cn) {
    FIO_LOG_INFO("  CN: '%.*s' (len=%zu)", (int)cert.subject_cn_len, cert.subject_cn, cert.subject_cn_len);
  } else {
    FIO_LOG_INFO("  CN: NULL");
  }
  ...
  while (fio_asn1_iterator_next(&tbs_it, &elem) == 0) {
    FIO_LOG_INFO("TBS[%d]: tag=0x%02X len=%zu is_context_3=%d", 
                 idx, elem.tag, elem.len, fio_asn1_is_context_tag(&elem, 3));
    
    if (fio_asn1_is_context_tag(&elem, 3)) {
      FIO_LOG_INFO("  Found extensions context tag [3]!");
      FIO_LOG_INFO("  elem.data[0..4]: 0x%02X 0x%02X 0x%02X 0x%02X",
                   elem.data[0], elem.data[1], elem.data[2], elem.data[3]);
      
      // Parse the inner SEQUENCE
      fio_asn1_element_s exts;
      if (fio_asn1_parse(&exts, elem.data, elem.len)) {
        FIO_LOG_INFO("  Extensions SEQUENCE: tag=0x%02X len=%zu", exts.tag, exts.len);
        ...
        while (fio_asn1_iterator_next(&exts_it, &ext) == 0) {
          FIO_LOG_INFO("  Extension[%d]: tag=0x%02X len=%zu", ext_idx, ext.tag, ext.len);
          ...
          if (fio_asn1_iterator_next(&ext_it, &oid) == 0) {
            FIO_LOG_INFO("    OID tag=0x%02X len=%zu", oid.tag, oid.len);
            
            // Print OID bytes
            char oid_str[64] = {0};
            int len = 0;
            for (size_t i = 0; i < oid.len && len < 60; i++) {
              len += snprintf(oid_str + len, sizeof(oid_str) - len, "%02X ", oid.data[i]);
            }
            FIO_LOG_INFO("    OID bytes: %s", oid_str);
            
            // Check if it's SAN
            if (fio_asn1_oid_eq(&oid, FIO_OID_SUBJECT_ALT_NAME)) {
              FIO_LOG_INFO("    -> This is SubjectAltName!");
            }
          }
          
          if (fio_asn1_iterator_next(&ext_it, &value) == 0) {
            FIO_LOG_INFO("    Value tag=0x%02X len=%zu", value.tag, value.len);
            ...
            if (fio_asn1_is_tag(&value, FIO_ASN1_OCTET_STRING)) {
              ...
              if (fio_asn1_parse(&san_seq, value.data, value.len)) {
                FIO_LOG_INFO("    Inner: tag=0x%02X len=%zu", san_seq.tag, san_seq.len);
                ...
                while (fio_asn1_iterator_next(&san_it, &gn) == 0) {
                  FIO_LOG_INFO("      GeneralName: tag=0x%02X len=%zu", gn.tag, gn.len);
                  FIO_LOG_INFO("      is_context_2=%d", fio_asn1_is_context_tag(&gn, 2));
                  if (gn.len > 0 && gn.len < 64) {
                    FIO_LOG_INFO("      data: '%.*s'", (int)gn.len, gn.data);
                  }
                }
              }
            }
