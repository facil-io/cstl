...
int main(void) {
...
#if !DEBUG
  fprintf(stderr, "\n");
  fio_test_hash_function(fio__aes128_gcm_speed_wrapper,
                         (char *)"fio AES-128-GCM",
                         7,
                         0,
                         0);
  fio_test_hash_function(fio__aes128_gcm_speed_wrapper,
                         ...
                         0);
  fprintf(stderr, "\n");
  fio_test_hash_function(fio__aes256_gcm_speed_wrapper,
                         (char *)"fio AES-256-GCM",
                         7,
                         0,
                         0);
  fio_test_hash_function(fio__aes256_gcm_speed_wrapper,
  ...
  fprintf(stderr, "\n\t* Comparing with OpenSSL:\n");
  {
    ...
    uint8_t *buffer = (uint8_t *)FIO_MEM_REALLOC(NULL, 0, buffer_len + 64, 0);
    uint8_t *out = (uint8_t *)FIO_MEM_REALLOC(NULL, 0, buffer_len + 64, 0);
    ...
    for (int i = 0; i < 1000; i++) {
      ...
      EVP_EncryptUpdate(ctx, out, &outlen, buffer, (int)buffer_len);
    ...
    double mb_per_sec =
        ((double)buffer_len * iterations) / (seconds * 1000000.0);
    fprintf(stderr,
            "\t\t%-40s %8.2f MB/s\n",
            "OpenSSL AES-128-GCM (8192 bytes)",
            mb_per_sec);
    ...
    mb_per_sec = ((double)buffer_len * iterations) / (seconds * 1000000.0);
    fprintf(stderr,
            "\t\t%-40s %8.2f MB/s\n",
            "OpenSSL AES-256-GCM (8192 bytes)",
            mb_per_sec);

    EVP_CIPHER_CTX_free(ctx);
    FIO_MEM_FREE(buffer, buffer_len + 64);
    FIO_MEM_FREE(out, buffer_len + 64);
  }
#endif /* HAVE_OPENSSL */
#endif /* !DEBUG */
